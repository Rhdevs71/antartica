...
local function extractCrystalInfo(obj)
    ...
    local hpDisplay = (maxHP > 0) and string.format("%s / %s", formatCompactNumber(curHP), formatCompactNumber(maxHP)) or "Bebas"
...
local function scanAndShowTopCrystal()
    ...
    WindUI:Notify({
        ...
        Content = string.format("Nama: %s\nHarga: %s\nHP: %s\nUkuran: %s\nBobot: %.1f KG\nLuck: +%.1f%%",
            top.Name, top.PriceDisplay, top.HPDisplay or "Bebas", top.SizeName, top.Weight, top.Luck),
        Duration = 6
    })
end
...
local function autoSnipeGlobalLoop()
    ...
    for _, c in ipairs(crystals) do
        ...
        local passHP = (State.SnipeMaxHP == math.huge) or (c.MaxHP == 0) or (c.MaxHP <= State.SnipeMaxHP)

        if passPrice and passSize and passLuck and passHP then
...
task.spawn(function()
    while true do
        ...
        if State.AutoMineMostExpensive and not State.IsReturning and not State.IsSniping then
            local crystals = findCrystalsInMap()
            if #crystals > 0 then
                for _, targetCrystal in ipairs(crystals) do
                    local passHP = not State.ApplyMaxHPToAutoMine or (State.SnipeMaxHP == math.huge) or (targetCrystal.MaxHP == 0) or (targetCrystal.MaxHP <= State.SnipeMaxHP)
                    if passHP then
...
BagTab:Dropdown({
    ...
    Desc = "Filter batas maksimal HP kristal yang diambil",
...
BagTab:Toggle({
    ...
    Desc = "Batasi HP kristal termahal sesuai batas HP yang dipilih",
    Default = true,
